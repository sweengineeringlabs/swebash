...
impl ReadlineConfig {
    /// Load configuration from `~/.{app_name}rc`.
    ///
    /// For example, `ReadlineConfig::load("swebash")` reads `~/.swebashrc`,
    /// while `ReadlineConfig::load("rdl")` reads `~/.rdlrc`.
    pub fn load(app_name: &str) -> Self {
        let rc_filename = format!(".{}rc", app_name);

        let config_path = std::env::var_os("HOME")
            .map(PathBuf::from)
            .or_else(dirs::home_dir)
            .map(|h| h.join(&rc_filename))
            .unwrap_or_else(|| PathBuf::from(&rc_filename));

        if let Ok(content) = std::fs::read_to_string(&config_path) {
            if let Ok(config) = toml::from_str::<RcFile>(&content) {
