...
fn list_short(path: Option<&str>) {
    let len = match path {
        Some(p) => unsafe { crate::spi::host::host_list_dir(p.as_ptr(), p.len()) },
        None => {
            let dot = b".";
            unsafe { crate::spi::host::host_list_dir(dot.as_ptr(), 1) }
        }
    };
...
fn list_long(path: Option<&str>) {
    ...
    for entry in listing.lines() {
        ...
        let stat_len = unsafe {
            crate::spi::host::host_stat(full_path.as_ptr(), full_path.len())
        };

        let mut is_dir = false;

        if stat_len > 0 {
            let stat_data = crate::api::buffer::read_response(stat_len as usize);
